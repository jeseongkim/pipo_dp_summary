...
sboxes = [ sb.SBOX_DICTIONARY['3-bit']['PIPOS3'], sb.SBOX_DICTIONARY['5-bit']['PIPOS51'], sb.SBOX_DICTIONARY['5-bit']['PIPOS52'], sb.SBOX_DICTIONARY['8-bit']['PIPOS8'] ]
...
def dpt_to_zeros(dpt):
	...
	LSB of in ~ MSB of in || LSB of out ~ MSB of out
	...
	zeros = []
	num2str = lambda x,n: "".join([str((x>>k)&1) for k in reversed(range(n))]) # num to str reverse-order respect to bin()
...
file_dir = os.getcwd()
file_name = "ineq_reverse.py"

# For each sbox, make ineq
for sbox in sboxes:
	dpt = sbox.get_DPT()[0]
	ones = dpt_to_zeros(dpt)
	mini = list(qm.simplify(ones, num_bits = sbox.inbit + sbox.outbit)) # make ineq
