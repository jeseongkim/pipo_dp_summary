...
importfolders = ["ineq", "mymodule"]
# Add directory of modules into sys.path
for foldername in importfolders:
	sys.path.append( os.path.abspath(os.getcwd() + '/' + foldername) )

try:
	from ineq import S3K, S51K, S52K, S8K  # S3K, S51K, S52K, S8K
...
def piposboxConstr(model , inVar , outVar, rounds, col):
	# S3
	outS3 = model.addMVar(shape = 3, vtype = GRB.BINARY, name 					= "(%d,%d)/temp1"%(rounds,col))
	sboxConstr(model, inVar[:3], outS3, ineqs["S3K"], constrname				= "(%d,%d)/S3"%(rounds,col))

	# S5_1
	outS51 = model.addMVar(shape = 5, vtype = GRB.BINARY, name 					= "(%d,%d)/temp2"%(rounds,col))
	sboxConstr(model, inVar[3:], outS51, ineqs['S51K'], constrname				= "(%d,%d)/S51"%(rounds,col))
	
	# Extended XOR
	# Copy output 3-bit of S3
	outS3c1 = model.addMVar(shape = 3, vtype = GRB.BINARY, name					= "(%d,%d)/cpy1"%(rounds,col))
	outS3c2 = model.addMVar(shape = 3, vtype = GRB.BINARY, name					= "(%d,%d)/cpy2"%(rounds,col))
	model.addConstr( outS3 == outS3c1 + outS3c2, name 							= "(%d,%d)/copyS3"%(rounds,col))

	extendedxor = model.addMVar(shape = 3, vtype = GRB.BINARY, name				= "(%d,%d)/temp3"%(rounds,col))
	model.addConstr( extendedxor == outS51[[1,4,0]] + outS3c1, name				= "(%d,%d)/ExXOR"%(rounds,col))

	# S5_2
	inS52 = model.addMVar(shape = 5, vtype = GRB.BINARY, name					= "(%d,%d)/temp4"%(rounds,col))
	model.addConstr(inS52[:3] + outVar[[0,6,5]] == extendedxor[[1,2,0]], name	= "(%d,%d)/cpyS52-1"%(rounds,col))
	model.addConstr(inS52[3:] == outS51[[2,3]], name							= "(%d,%d)/cpyS52-2"%(rounds,col))
	outS52 = model.addMVar(shape = 5, vtype = GRB.BINARY, name 					= "(%d,%d)/temp5"%(rounds,col))
